...
const handelCreateComment = async (req , res) => {
    ...
    if(!req.body.rate) return res.status(400).json({massage : "لطفا امتیاز ثبت کنید"})
    const decodedToken = JWT.decode( req.headers.authorization, process.env.SECRETKEY )
    const user = await User.findOne({_id : decodedToken.userId})
    const newComment = new Comment({ title: req.body.title , name : user.firstName , desc: req.body.desc , rating: req.body.rate , createdBy: req.body.createdBy , idOfProduct: req.body.idOfProduct })
